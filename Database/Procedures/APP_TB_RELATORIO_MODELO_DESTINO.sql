-- Inicio do Arquivo dbo.DATA_TB_RELATORIO_MODELO_DESTINO

use Program
go

--------------------------------------------------------------------------------
print '*** dbo.DATA_TB_RELATORIO_MODELO_DESTINO - Data de Execução: ' + FORMAT(GETDATE(),'dd/MM/yyyy HH:mm:ss')
--------------------------------------------------------------------------------
go

CREATE OR ALTER PROCEDURE dbo.DATA_TB_RELATORIO_MODELO_DESTINO
 	@p_COD_MODELO_DESTINO int = 0 output
,	@p_COD_RELATORIO_MODELO int
,	@p_COD_DESTINO_TIPO int
,	@p_FL_NOTIFICAR_SOLICITANTE bit = Null
...
BEGIN TRY
	...
	if (isnull(@p_OPERACAO, 'X') != 'D') begin
		if exists (
			...
			select	1
			from	Report.DBO.TB_RELATORIO_MODELO_DESTINO	with (nolock)
			where	COD_MODELO_DESTINO = @p_COD_MODELO_DESTINO

		) begin

			update	Report.DBO.TB_RELATORIO_MODELO_DESTINO
			set	COD_RELATORIO_MODELO = @p_COD_RELATORIO_MODELO
			,	COD_DESTINO_TIPO = @p_COD_DESTINO_TIPO
			,	FL_NOTIFICAR_SOLICITANTE = @p_FL_NOTIFICAR_SOLICITANTE
			...
			insert	Report.dbo.TB_RELATORIO_MODELO_DESTINO(
				COD_RELATORIO_MODELO
			,	COD_DESTINO_TIPO
			,	FL_NOTIFICAR_SOLICITANTE
			...
			set	@RETORNO=@@ERROR
			set	@p_COD_MODELO_DESTINO = SCOPE_IDENTITY()

		end
	end else begin
		-- Operação de exclusão (D)
		-- Verificar se ID é obrigatório para operação D
		IF @p_COD_MODELO_DESTINO IS NULL OR @p_COD_MODELO_DESTINO <= 0
		BEGIN
			SET @RETORNO = 1
		...
		if exists (
			...
			delete	Report.DBO.TB_RELATORIO_MODELO_DESTINO
			where	COD_MODELO_DESTINO = @p_COD_MODELO_DESTINO
...
-- Fim do Arquivo dbo.DATA_TB_RELATORIO_MODELO_DESTINO
...
--------------------------------------------------------------------------------
print '*** dbo.RULE_TB_RELATORIO_MODELO_DESTINO - Data de Execução: ' + FORMAT(GETDATE(),'dd/MM/yyyy HH:mm:ss')
--------------------------------------------------------------------------------
go

CREATE OR ALTER PROCEDURE dbo.RULE_TB_RELATORIO_MODELO_DESTINO
 	@p_COD_MODELO_DESTINO int = 0 output
...
return	@RETORNO
GO
-- Fim do Arquivo dbo.RULE_TB_RELATORIO_MODELO_DESTINO
...
--------------------------------------------------------------------------------
print '*** dbo.APP_TB_RELATORIO_MODELO_DESTINO - Data de Execução: ' + FORMAT(GETDATE(),'dd/MM/yyyy HH:mm:ss')
--------------------------------------------------------------------------------
go

CREATE OR ALTER PROCEDURE dbo.APP_TB_RELATORIO_MODELO_DESTINO
 	@p_COD_MODELO_DESTINO int = 0 output
,	@p_COD_RELATORIO_MODELO int = Null
,	@p_COD_DESTINO_TIPO int = Null
,	@p_FL_NOTIFICAR_SOLICITANTE bit = Null
...
BEGIN TRY
	...
	execute @RETORNO = dbo.RULE_TB_RELATORIO_MODELO_DESTINO
	  @p_COD_MODELO_DESTINO = @p_COD_MODELO_DESTINO output
	, @p_COD_RELATORIO_MODELO = @p_COD_RELATORIO_MODELO
	, @p_COD_DESTINO_TIPO = @p_COD_DESTINO_TIPO
	, @p_FL_NOTIFICAR_SOLICITANTE = @p_FL_NOTIFICAR_SOLICITANTE
	...
	execute @RETORNO = dbo.DATA_TB_RELATORIO_MODELO_DESTINO
	  @p_COD_MODELO_DESTINO = @p_COD_MODELO_DESTINO output
	, @p_COD_RELATORIO_MODELO = @p_COD_RELATORIO_MODELO
	, @p_COD_DESTINO_TIPO = @p_COD_DESTINO_TIPO
	, @p_FL_NOTIFICAR_SOLICITANTE = @p_FL_NOTIFICAR_SOLICITANTE
...
ELSE
	RETURN @RETORNO
go
-- Fim do Arquivo dbo.APP_TB_RELATORIO_MODELO_DESTINO
